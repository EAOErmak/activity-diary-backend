...
3. Keep secrets only in `.env` or `.env.local`. Do not commit them.
...
- The current local IntelliJ run configuration uses the `web` profile. Keep using `web` if you want the same behavior after the migration.
- Set the active profile in IntelliJ to `web` or `desktop`, or pass a program argument such as `--spring.profiles.active=web`.
