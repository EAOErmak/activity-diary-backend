...
@Service
public class PfcPerEatingChartStrategy implements ChartCalculationStrategy {

    private final FoodChartSupportService foodChartSupportService;

    public PfcPerEatingChartStrategy(FoodChartSupportService foodChartSupportService) {
        this.foodChartSupportService = foodChartSupportService;
    }

    @Override
    public ChartResponseDto calculate(Long userId, ChartFilterDto filter) {
        return foodChartSupportService.buildPfcPerDiary(getChartType(), userId, filter);
    }

    @Override
    public ChartType getChartType() {
        return ChartType.PFC_PER_EATING;
    }
}
